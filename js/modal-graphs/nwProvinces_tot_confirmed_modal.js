...
const NW_Date_Modal = [];
const NW_Province_Modal = [];
const NW_Death_Modal = [];
nwProvinceChart();

async function nwProvinceChart() {
  await getNwChart();
  // await getNwDeathStats();
  const ctx = document.getElementById("nwChartModalConfirmed").getContext("2d");
  const myChart = new Chart(ctx, {
    type: "line",
    data: {
      labels: NW_Date_Modal,
      type: "line",
      defaultFontFamily: "Montserrat",
      datasets: [
        {
          label: "TotalConfirmed Cases",
          data: NW_Province_Modal,
...
nwProvinceDeathsChart();

async function nwProvinceDeathsChart() {
  // await getNwChart();
  await getNwDeathStats();
  const ctx = document.getElementById("nwChartModalDeaths").getContext("2d");
  const myChart = new Chart(ctx, {
    type: "line",
    data: {
      labels: NW_Date_Modal,
...
async function getNwChart() {
  ...
  table.forEach(row => {
    ...
    const nwDate = columns[0];
        NW_Date_Modal.push(nwDate);
    const nwProvinceData = columns[9];
        NW_Province_Modal.push(nwProvinceData);
...
async function getNwDeathStats() {
  ...
  table.forEach(row => {
    ...
    //     EC_Date.push(ecDate);
    const nwProvinceDeathsData = columns[9];
        NW_Death_Modal.push(nwProvinceDeathsData);
